  ...
  // a staticly allocated neural network with a generic parameter type the weights can only hold 3 valuers -1 0 1
  template <typename T, int inputs, int outputs> struct StaticLayerTrinary {
    T weights[outputs][inputs];
    T bias[outputs];

    
    void init(std::mt19937 &rng) {
      // Uniform integer distribution from 0 to 4
      std::uniform_int_distribution<int> dist(0, 4);
      
      // Map to -1, 0, 1
      const T values[5] = {-2, -1, 0, 1, 2};
      
      for (int i = 0; i < outputs; ++i) {
          for (int j = 0; j < inputs; ++j) {
              weights[i][j] = values[dist(rng)];
          }
          bias[i] = values[dist(rng)];
      }
    }
      ...
      for (int i = 0; i < outputs; ++i) {
          for (int j = 0; j < inputs; ++j) {
              if (chance(rng) < mutation_rate) {
                int weight_change = chance(rng) < 0.5 ? 1 : -1;
                weights[i][j] += weight_change;
                // Clip the weight to -2 to 2
                if (weights[i][j] < -2) {
                    weights[i][j] = -2;
                } else if (weights[i][j] > 2) {
                    weights[i][j] = 2;
                }
              }
          }
          int bias_change = chance(rng) < 0.5 ? 1 : -1;
          bias[i] += bias_change;

          // Clip the bias to -2 to 2
  ...
  struct StaticNeuralNetTrinary {
    static_assert(hidden_count >= 1, "StaticNeuralNet must have at least one hidden layer");

    StaticLayerTrinary<T, inputs, hidden_size> input_layer;
    StaticLayerTrinary<T, hidden_size, hidden_size> hidden_layers[hidden_count - 1];
    StaticLayerTrinary<T, hidden_size, outputs> output_layer;
